...
1. `password` - to reperesent a password field. This field type does not except a default value, the value is being generated on its own. This field is being saved and updated bcrypt encription, based on the SECRET. This field type can have the following options:
   - `min_length` - the minimum length of the paragraph (in characters number)
   - `max_length` - the maximum length of the paragraph (in characters number)
