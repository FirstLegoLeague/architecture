...
For a module to require access to the language files of the system, it must add the `language` string to the `require`
array in the `module.yml`. A module that have required the language files, would have an evnironment variable
`LANG_FILE` as part of its process environment variables. This variable is the absolute path to the language file
selected by the user at installation time. The language file is a `JSON` file containing the compatible to [webtranslateit](https://webtranslateit.com/en)
json format. The filename can be split into the directory in which the file is stored, and the filename which is the locale
used by the system.
