...
`$FLL_SYSTEM/modules/$MODULE_NAME`. Inside this folder there should be a file `module.yml` containing the module
...
language: # optional
  $language-frases$          # See language section for more information
```
...
The termination of the process must be started after the process have got a `SIGTERM` from the operating system. In
...
prior to the signal, might cause the launcher to shut down the process by force. The process should treat `SIGINT` the
same way as it treat `SIGTERM`.
...
provide different executable for each operating system. In the field `executable` one can switch a 
with an
...
promised to support only `win32` and `darwin` operation systems.
...
For a module to require access to the language files of the system, it must add the `language` property to the `module.yml` file.
This property will contain an array of frases the module requires to translate. A module that have required the language files,
would have an evnironment variable `LANG_FILE` as part of its process environment variables. This variable is the absolute path
to the language file selected by the user at installation time. The language file is a `JSON` file containing the compatible
to [i18njs](http://i18njs.com/) json format. The filename can be split into the directory in which the file is stored, and
the filename which is the locale used by the system.
...
A module can recieve his configurations from the launcer by using a mhub. To aquire the configurations a module must
must subscribe to the mhub topic family `config:*` and listen to any messages on this topic. The username for this
topic family will be `configuration` and the password will be the `SECRET` environment variable.

The message on the `config:*` topic family will be `config:update`. When the field is changed, either by the module or
by the centrelized configuration module, the updating party must send an mhub message on this topic. The data of this
message must follow this scheme:
```json
{
  "module": "$MODULE_NAME$",
  "fields": [
...
There is no restriction to how much fields can be send in an update message. A launcher may or may not send all of the
fields updatesof the same module he currently own  in one message. Therefore a module must be prepare to handle
multiple messages revieced in a short period.
