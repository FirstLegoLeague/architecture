...
2. `root` (optional) - A relative path from the repository root directory to the directory that should be exposed. defaults to `./`.
...
1. `password` - to reperesent a password field. This field type does not except a default value,
    the value is being generated on its own.
