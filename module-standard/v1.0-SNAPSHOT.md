...
authorization of the user. The data must include a `username` field.

_**TODO add a schema for the jwt**_
...
The system has a central UI for managing user controlled configuration. If a service wants to apply to this change it has to subscribe to the Mhub topic family `config:*` and listenr to any messages on this topic. The username for this topic family will be `configuration` and the password will be the `SECRET` environment variable. The types of messages on the `config:*` topic family will be:
1. `config:get_fields` - a request sent by the configuration service to ask for the fields each service wishes to subscribe. This message has no data.
1. `config:fields` - after the `config:get_fields` is sent, the module must send the fields it wishes to allow the user to configure, arranged into different `fields-families` in a `config:fields` message. The data of this message must follow this scheme:
```json
{
  "module": "$MODULE_NAME$",
  "fields": {
...
```
The configuration module will the display the fields for the user to see, grouped by their `fields-family`. A field must contain either a `type` property or a `values` property, but not both. `$FIELD_TYPE$` values can be `short text`, `long text`, `integer`, `decimal`, `date`, `datetime`, `boolean`.
1. `config:set_field` - when the field is changed, either by the module or by the centrelized configuration UI, the updating party must send an Mhub message `config:set_field` with the data to update the other modules. The data of this message must follow this scheme:
```json
{
  "module": "$MODULE_NAME$",
  "field-family" "$FIELD_FAMILY_NAME$",
  "field": "$FIELD_NAME$",
